...
def run(
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            # DB操作終了
            cur.close()
            for iiii in range(len(db_lis_last)):
                if 'Stop' in db_lis_last[iiii][2]:
                    cur = conn.cursor(buffered=True)
                    sql = ("UPDATE spots SET status = %s WHERE id = %s")
                    param = ('None',db_lis_last[iiii][0])
            ...
            if det is not None and len(det):
                ...
                # DB操作終了
                cur.close()                
                for ii in range(len(db_lis_last)):
                    if 'Stop' in db_lis_last[ii][2]:
                        cur = conn.cursor(buffered=True)
                        sql = ("UPDATE spots SET status = %s WHERE id = %s")
                        param = ('None',db_lis_last[ii][0])
                        cur.execute(sql,param)
                        conn.commit()
                        cur.close()
                        shutil.rmtree(project)
